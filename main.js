...
import { 
    ...
    'consistent_coder': { name: 'Consistent Coder', description: 'Maintain a 7-day study streak.' }
        ...
        let saveSessionFunction, getLeaderboardFunction, addPlannerTaskFunction, updatePlannerTaskFunction, deletePlannerTaskFunction, createGroupFunction, joinGroupFunction;
...
let pauseStartTime = 0;
        ...
        getLeaderboardFunction = httpsCallable(functions, 'getLeaderboard');
        ...
        async function saveSession(subject, durationSeconds, sessionType = 'study') { // Default to 'study'
            ...
            try {
                ...
                // Re-fetch daily totals from Firestore after the backend update to ensure UI consistency
                const userDocRef = doc(db, 'artifacts', appId, 'users', currentUser.uid);
                const userDoc = await getDoc(userDocRef);
                if (userDoc.exists()) {
                    const data = userDoc.data();
                    const todayStr = getCurrentDate().toISOString().split('T')[0];
                    totalTimeTodayInSeconds = (data.totalTimeToday?.date === todayStr) ? data.totalTimeToday.seconds : 0;
                    totalBreakTimeTodayInSeconds = (data.totalBreakTimeToday?.date === todayStr) ? data.totalBreakTimeToday.seconds : 0;
                    updateTotalTimeDisplay();
                }

                showToast(`Session of ${formatTime(cappedDuration, false)} saved!`, "success");

                // Display achievement toasts for any achievements unlocked and returned by the backend
    ...
    achievementsGrid.innerHTML = Object.keys(ACHIEVEMENTS).map(key => {
        ...
        async function addPlannerTask(title, dueDateStr = null) {
            ...
            try {
                ...
                console.error("Error adding task via function:", error);
        ...
        async function updatePlannerTask(taskId, data) {
            ...
            // Ensure Date objects are converted to a string format the backend can parse
            ...
            try {
                await updatePlannerTaskFunction({
                    taskId: taskId,
                    updateData: updateData,
                    appId: appId
                });
            } catch (error) {
                console.error("Error updating task via function:", error);
        ...
        async function deletePlannerTask(taskId) {
            if (!currentUser || !taskId) return;
            try {
                await deletePlannerTaskFunction({ taskId: taskId, appId: appId });
                plannerState.selectedTaskId = null;
                renderPlannerTaskDetails(); // Hide the panel
            } catch (error) {
                console.error("Error deleting task via function:", error);
        ...
        async function renderLeaderboard(period = 'weekly', containerId = 'ranking-list') {
            ...
            try {
                const result = await getLeaderboardFunction({ period: period, appId: appId });
                const userScores = result.data; // Data comes sorted from the backend
                ...
                showToast(error.message || 'Could not load leaderboard.', 'error');
                container.innerHTML = `<div class="empty-group"><i class="fas fa-exclamation-triangle"></i><h3>Error loading ranks</h3><p>Please try again later.</p></div>`;
            }
        }
        ...
        async function joinGroup(groupId, password = null) {
            if (!currentUser) return;
            try {
                await joinGroupFunction({ groupId, password, appId });
                showToast("Group joined successfully!", "success");
            } catch (error) {
                console.error('Error joining group via function:', error);
        ...
        async function renderGroupLeaderboard(period) {
            const container = document.getElementById('group-ranking-list');
            if (!container) return;
            ...
            try {
                ...
                console.error(`Error fetching group leaderboard for group ${currentGroupId}:`, error);
                showToast(error.message || 'Could not load group leaderboard.', 'error');
                container.innerHTML = `<div class="empty-group"><i class="fas fa-exclamation-triangle"></i><h3>Error loading ranks</h3><p>Please try again later.</p></div>`;
            }
        }
        ...
        ael('create-group-done-btn', 'click', async () => {
            ...
            try {
                await createGroupFunction({
                    name, password, category, timeGoal, capacity, description, appId
                });
