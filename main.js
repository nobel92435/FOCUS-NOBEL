        ...
        function initializeFirebase() {
            try {
                ...
                onAuthStateChanged(auth, async (user) => {
                    if (user) {
                        currentUser = user;
                        // Initial data load and listeners can be triggered from here
                    } else {
                        currentUser = null;
                ...
                if (typeof __initial_auth_token !== 'undefined' && __initial_auth_token) {
                    signInWithCustomToken(auth, __initial_auth_token).catch(err => console.error("Custom token sign-in failed", err));
                } else {
