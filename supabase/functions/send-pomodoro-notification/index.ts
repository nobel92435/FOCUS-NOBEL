...
// Handles Pomodoro phase transitions by logging sessions, updating status,
// and triggering push notifications for subscribed users.
...
function buildCorsHeaders(originHeader: string | null) {
  ...
  const allowOrigin = normalizedOrigin && allowedOrigins.map(normalizeOrigin).includes(normalizedOrigin)
    ? normalizedOrigin
    : allowedOrigins[0];
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const rawSubscription = profile?.push_subscription;

    if (rawSubscription) {
      const subscription = typeof rawSubscription === "string"
        ? JSON.parse(rawSubscription)
        : rawSubscription;
      const pushSubscriber = PushSubscriber.fromJSON(subscription);

      await pushSubscriber.pushTextMessage(
        appServer,
...
async function handlePhaseEndOnServer(supabase: any, userId: string, oldState: string) {
...
async function updateUserStatusForNewPhase(supabase: any, userId: string, newState: string) {
