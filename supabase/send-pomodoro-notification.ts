// Supabase Edge Function: send-pomodoro-notification (revised)
  ...
  try {
    ...
    if (raw) {
      try {
        ...
        await sub.pushTextMessage(appServer, JSON.stringify({
          title,
          body
        }));
...
async function updateUserStatusForNewPhase(supabase: any, userId: string, newState: string) {
  ...
  let newStudyingStatus: Record<string, string> | null = null;
