...
async function routeAfterAuth() {
    ...
    // Not logged in or malformed ID → stay on auth screen
...
achievementsGrid.innerHTML = Object.keys(ACHIEVEMENTS).map(key => {
    ...
    function __mergeLiveVirtual(norm){
      ...
      // don’t double-count if a session row is already being written continuously;
    ...
    async function __startStatsRealtime() {
      ...
      // subscribe to my user row to track studying start/stop and subject/type flips (normal vs pomodoro)
      if (__stats.subUser) supabase.removeChannel(__stats.subUser);
      __stats.subUser = supabase
        .channel(`users:me:${currentUser.id}`)
        .on('postgres_changes',
    ...
    function __makeInsightGenerator(today){
      return function generateAIInsight(sessions, elementId='ai-insight-text'){
        ...
        const cands = [
          ...
          ()=>{ const days=new Set(); study.forEach(s=>{ if ((today - s.startTime)/86400000 <= 7) days.add(s.startTime.getDay()); }); if (days.size>=5) return `Great consistency! You studied on ${days.size} days this week.`; if (days.size>0&&days.size<3) return 'Build the habit — aim for short daily sessions.'; return null; }
        ];
        ...
        if (!msg) {
          ...
          msg = top ? `You've been focusing well on '${top[0]}'. Consistency is building!` : 'Keep going — more sessions unlock deeper insights.';
        }
        el.textContent = msg;
      };
    }
    ...
    function __renderSessionLog(list, containerId) {
      ...
      // delegate actions (delete/edit) — uses your existing deleteSession handler
    ...
    async function renderJoinedGroups() {
        ...
        if (mErr) { console.error(mErr); container.innerHTML = `<div class="text-center text-red-400 py-6">Failed to load your groups.</div>`; return; }

        const joinedGroupIds = (memRows || []).map(r => r.group_id);
        currentUserData.joined_groups = joinedGroupIds; // keep UI compatible
        ...
        if (gErr) { console.error(gErr); container.innerHTML = `<div class="text-center text-red-400 py-6">Failed to load your groups.</div>`; return; }

        const todayStr = new Date().toISOString().split('T')[0];
        const sinceISO = new Date(Date.now() - 24*3600*1000).toISOString();

        // Preload member counts and ids per group
        ...
        for (const g of groups) {
          const { data: rows } = await supabase
            ...
            .eq('group_id', g.id);
          membersByGroup[g.id] = (rows || []).map(r => r.profile_id);
        }

        const enriched = await Promise.all(groups.map(async (g) => {
          const memberIds = membersByGroup[g.id] || [];
          let totalSecondsToday = 0, studiedToday = 0;

          if (memberIds.length) {
            const { data: users } = await supabase
              .from('users')
              .select('id,total_time_today')
              .in('id', memberIds);
            (users || []).forEach(u => {
              const ttt = u?.total_time_today;
              if (ttt?.date === todayStr) {
                const s = Number(ttt?.seconds || 0);
                totalSecondsToday += s;
                if (s > 0) studiedToday++;
              }
            });
          ...
          const { count: newMsgs } = await supabase
            .from('group_messages')
            .select('id', { count: 'exact', head: true })
            .eq('group_id', g.id)
            .gte('created_at', sinceISO);

          const attendance = memberIds.length ? Math.round((studiedToday / memberIds.length) * 100) : 0;
          const avgTime = memberIds.length ? totalSecondsToday / memberIds.length : 0;

          return { ...g, membersCount: memberIds.length, attendance, avgTime, newMessages: newMsgs || 0 };
        }));
        ...
        container.innerHTML = '<div class="space-y-4">' + enriched.map(group => {
          ...
          const leader = pick(group.leaderName, group.leader_name, '—');
    ...
    async function renderGroupRankings() {
        ...
        if (gErr) { console.error(gErr); container.innerHTML = `<div class="text-center text-red-400 py-6">Failed to load groups.</div>`; return; }

        // Build member ids by group
        const membersByGroup = {};
        for (const g of groups) {
          const { data: rows } = await supabase
            .from('group_members')
        ...
        const enriched = await Promise.all(groups.map(async (g) => {
          ...
          if (memberIds.length) {
            const { data: users } = await supabase
              .from('users')
              .select('id,total_study_seconds')
              .in('id', memberIds);
            totalStudyTime = (users || []).reduce((s, u) => s + Number(u.total_study_seconds || 0), 0);
          }
          const mockAttendance = Math.floor(Math.random() * 31) + 70; // keep your placeholder
          return { ...g, memberIds, totalStudyTime, attendance: mockAttendance };
        }));

        // Filters
        let filtered = enriched.filter(g => {
          if (filters.univ && g.category !== 'University') return false;
          if (filters.available && g.memberIds.length >= g.capacity) return false;
        ...
        filtered.sort((a, b) => {
          switch (sortMethod) {
            case 'attendance': return (b.attendance - a.attendance) || (b.memberIds.length - a.memberIds.length);
            case 'new': {
            ...
            case 'studytime':
            default: {
              const avgA = a.totalStudyTime / (a.memberIds.length || 1);
              const avgB = b.totalStudyTime / (b.memberIds.length || 1);
              return (avgB - avgA) || (b.memberIds.length - a.memberIds.length);
            }
          }
        });
        ...
        container.innerHTML = '<div class="flex flex-col gap-4">' + filtered.map((g, idx) => {
          ...
          const avgPerMember = g.totalStudyTime / (memberCount || 1);
          const isFull = memberCount >= g.capacity;
          ...
          const leaderName = pick(g.leaderName, g.leader_name, '—');
            ...
            <div class="group-card ranking bg-gray-800 rounded-xl overflow-hidden shadow-lg" data-group-id="${g.id}">
              <div class="p-4">
                ...
                <div class="grid grid-cols-3 gap-x-2 gap-y-1 text-xs mb-3">
                  ...
                  <span class="text-gray-400 col-span-2">Members <b class="text-white">${memberCount}/${g.capacity} people</b></span>
                  ...
                  <span class="text-gray-400 col-span-2">Time <b class="text-white">${formatTime(avgPerMember, false)}</b></span>
        ...
        document.querySelectorAll('.join-btn').forEach(btn => {
          btn.addEventListener('click', async function() {
            ...
            if ((currentUserData?.joined_groups || []).includes(groupId)) { showToast("You've already joined this group.", 'info'); return; }
            if (this.disabled) { showToast("This group is currently full.", "info"); return; }
            ...
            if (isPrivate) {
              ...
              document.getElementById('password-prompt-form').onsubmit = async (e) => {
                ...
                if (!g || password !== g.password) { showToast('Incorrect password', 'error'); return; }
                modal.classList.remove('active');
        ...
        document.querySelectorAll('.group-card.ranking').forEach(card => {
          card.addEventListener('click', (e) => {
            ...
            } else showToast('Join the group to see details.', 'info');
          });
        });
      }          
      ...
      async function joinGroup(groupId) {
        ...
        // refresh whichever UI you’re on
                    ...
                    return `
                        <div class="member-list-card member-profile-link" data-user-id="${member.id}">
                            ...
                            ${!isStudying && member.id !== currentUser.id ? `
                                <button class="wake-up-btn ml-2 px-3 py-1.5 bg-sky-500 text-white rounded-full text-xs font-semibold hover:bg-sky-600 transition flex items-center gap-1.5" data-target-user-id="${member.id}" data-target-user-name="${member.username || 'Anonymous'}">
    ...
    function renderGroupStudiconView() {
        ...
        container.innerHTML = `
            <div class="p-4 grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4">
                ${membersArray.map(member => {
                    if (!member) return ''; // Add a guard for null/undefined members
                    const isStudying = member.studying && member.studying.type === 'study';
                    const studiconUrl = member.studicon_url || 'https://api.dicebear.com/8.x/miniavs/svg?seed=default';
                    const subject = isStudying ? member.studying.subject : 'Idle';
                    return `
                        <div class="studicon-member-card text-center cursor-pointer" data-user-id="${member.id}">
                            <div class="relative w-24 h-24 mx-auto">
                                ...
                                ${isStudying ? `<div class="absolute bottom-0 right-0 w-6 h-6 bg-green-500 border-2 border-gray-900 rounded-full"></div>` : ''}
                            </div>
                            <p class="mt-2 font-semibold text-white truncate">${member.username || 'Anonymous'}</p>
                            <p class="text-xs ${isStudying ? 'text-green-400' : 'text-gray-400'} truncate">${subject}</p>
                        </div>
                    `;
                }).join('')}
            </div>
      ...
      function setupGroupMemberListeners(memberIds) {
        ...
        memberIds.forEach(async (memberId) => {
          ...
          const { data: sRows } = await supabase
            .from('sessions')
            .select('id,subject,durationSec,endedAt,type')
